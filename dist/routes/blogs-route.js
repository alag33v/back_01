...
exports.blogsRoute.post("/", auth_middleware_1.authMiddleware, (0, blog_validators_1.blogValidator)(), (req, res) => {
    const { name, description, websiteUrl } = req.body;
    const createdBlog = blog_repository_1.BlogRepository.createBlog(name, description, websiteUrl);
    res.status(201).send(createdBlog);
});
exports.blogsRoute.put("/:id", auth_middleware_1.authMiddleware, (0, blog_validators_1.blogValidator)(), (req, res) => {
    const blog = blog_repository_1.BlogRepository.getBlogById(req.params.id);
    if (!blog) {
        res.sendStatus(404);
        return;
